...
main(int argc, char *argv[]) {
	if (argc != 4) {
		fprintf(stderr, "Usage: ./jbc_build [temp_dir_name] [btree_file_name] [btree_building_time]\n");
		return -1;
	}
	...
	if ((btree = open(argv[2], O_CREAT | O_TRUNC | O_WRONLY, 0766)) < 0) {
	...
	for (cur_pos = HEAD_SIZE, rdt = opt = wrt = i = 0; i < TEMP_N; ++i) {
		...
		fflush(stdout);
		if ((temp = open(TEMP_FILE_NAME, O_CREAT | O_TRUNC | O_WRONLY, 0666)) == -1) {
		...
		if ((temp = open(TEMP_FILE_NAME, O_RDONLY)) == -1) {
			fprintf(stderr, "Failed to open temp file \"%s\" for reading\n", TEMP_FILE_NAME);
			return -1;
		}
		...
		head.btreeinfo[i].itemn = itemn;
		head.btreeinfo[i].start = cur_pos;
		head.btreeinfo[i].blockn = itemn / ELEM_NUM + (itemn % ELEM_NUM != 0);
		...
		cur_pos += head.btreeinfo[i].blockn * BLK_SIZE;
		free(keypool);
		free(items);
		free(idx);
		gettimeofday(&end, NULL);
	...
	if (write(btree, &head, HEAD_SIZE) != HEAD_SIZE) {
