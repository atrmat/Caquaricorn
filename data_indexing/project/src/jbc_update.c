...
main(int argc, char *argv[]) {
	if (argc != 5) {
		fprintf(stderr, "Usage: ./jbc_update [operations_file_name] [old_btree_file] [new_btree_file] [update_time]\n");
		return -1;
	}
	...
	if (read(obtree, &ohead, HEAD_SIZE) != HEAD_SIZE) {
	...
	if ((nbtree = open(argv[3], O_CREAT | O_TRUNC | O_WRONLY, 0766)) < 0) {
	...
	for (cur_pos = HEAD_SIZE, opt = wrt = i = 0; i < TEMP_N; ++i) {
		...
		fflush(stdout);
		gettimeofday(&start, NULL);
		lseek64(obtree, ohead.btreeinfo[i].start, SEEK_SET);
		for (j = totlen = 0; j < ohead.btreeinfo[i].itemn; j += ELEM_NUM) {
		...
		fprintf(file, "| BTree %"PRId64" | RD %"PRId64" items %"PRId64"B %"PRId64"us ", i, ohead.btreeinfo[i].itemn, ohead.btreeinfo[i].blockn * BLK_SIZE, segt);
		rdt += segt;
		...
		fflush(stdout);
		gettimeofday(&start, NULL);
		MakeNewIndex(tmpindex, 0, 0, ohead.btreeinfo[i].itemn);
		for (j = 0; j < ohead.btreeinfo[i].itemn; ++j)
			nindex[tmpindex[j]] = j;
		buffer[0] = '\0';
		JSLF(PValue, PJSLArray[i], (uint8_t *)buffer);
		for (j = k = 0, on = 0; j < ohead.btreeinfo[i].itemn || PValue != NULL;) {
			if (PValue == NULL)
				items[k++] = tmpitems[nindex[j++]];
			else if (j == ohead.btreeinfo[i].itemn) {
				if (*PValue > 0) {
		...
		fflush(stdout);
		gettimeofday(&start, NULL);
		nhead.btreeinfo[i].start = cur_pos;
		nhead.btreeinfo[i].itemn = k;
		nhead.btreeinfo[i].blockn = k / ELEM_NUM + (k % ELEM_NUM != 0);
		...
		cur_pos += nhead.btreeinfo[i].blockn * BLK_SIZE;
		gettimeofday(&end, NULL);
		segt = (int64_t)(end.tv_sec - start.tv_sec) * 1000000 + end.tv_usec - start.tv_usec;
		mems = totlen + MAX(k, ohead.btreeinfo[i].itemn) * sizeof(int64_t) + MAX(k, 2 * ohead.btreeinfo[i].itemn) * sizeof(struct Item);
		fprintf(file, "| WR %"PRId64"B / %"PRId64"us | Mem %"PRId64"B+ %luB |\n", nhead.btreeinfo[i].blockn * BLK_SIZE, segt, mems, Bytes);
		wrt += segt;
	}
	...
	if (write(nbtree, &nhead, HEAD_SIZE) != HEAD_SIZE) {
