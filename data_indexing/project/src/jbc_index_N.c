...
main(int argc, char *argv[]) {
	if (argc != 6) {
		...
		// /root/cx_src/src/jbc_index_N /tmp/btree.bin n /tmp/data/index_sample.txt /tmp/result/jbc_index_out.txt /tmp/result/jit.txt
		return -1;
	}
	...
	// read the sample.txt into Judy array in the memory
	...
	if (use_cache)
		for (i = 0; i < TEMP_N; ++i) {
			lseek64(btree, head_N.btreen_info.start, SEEK_SET);
			read(btree, &cache[i], BLK_SIZE);
		}
	gettimeofday(&end, NULL);
	// tot is the total deal time 
	tot = (int64_t)(end.tv_sec - start.tv_sec) * 1000000 + end.tv_usec - start.tv_usec;
	buffer[0] = '\0';
	JSLF(PValue, PJSLArray, (uint8_t *)buffer); // start from the smallest string
	for (i = seekn = 0; PValue != NULL; ++i) {
		gettimeofday(&start, NULL);
		hash = BKDRHash((uint8_t *)buffer);// the buffer's itemn is hashed to an integer
		for (cur = 0; ; ) {
			if (cur >= ELEM_NUM)// if the position isn't in the root(cur == 0)
				read(btree, &block, BLK_SIZE);
			for (f = 0, t = MIN(ELEM_NUM, head_N.btreen_info.itemn - cur) - 1; f < t; ) {
				mid = f + ((t - f + 1) >> 1);
